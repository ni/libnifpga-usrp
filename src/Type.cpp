...
Type::Type(const size_t logicalBits,
           const size_t elementBytes,
           const bool   isSigned) :
   logicalBits(logicalBits),
   elementBytes(elementBytes),
   typeIsSigned(isSigned)
{
}
...
{
   return logicalBits  == other.logicalBits
      &&  elementBytes == other.elementBytes
...
{
   return isTargetToHostFifo(type)
       || isHostToTargetFifo(type);
}
