...
class Session
{
   public:
      ...
      template <typename T>
      void read(NiFpgaEx_Register  reg,
                typename T::CType& value) const;

      template <typename T>
      void write(NiFpgaEx_Register reg,
                 typename T::CType value) const;

      template <typename T>
      void readArray(NiFpgaEx_RegisterArray reg,
                     typename T::CType*     values,
                     size_t                 count) const;

      template <typename T>
      void writeArray(NiFpgaEx_RegisterArray   reg,
                      const typename T::CType* values,
                      size_t                   count) const;

      void acknowledgeIrqs(uint32_t irqs);

      void waitOnIrqs(uint32_t         irqs,
                      uint32_t         timeout,
                      uint32_t* const  irqsAsserted,
                      bool*            timedOut);

      void configureFifo(NiFpgaEx_DmaFifo fifo,
                         size_t           requestedDepth,
                         size_t*          actualDepth);

      void configureFifoGpu(NiFpgaEx_DmaFifo fifo,
                            size_t           depth,
                            void*            buffer);
      ...
      void releaseFifoElements(NiFpgaEx_DmaFifo fifo,
                               size_t           elements);
   ...
   private:
      ...
      void readOrWrite(NiFpgaEx_Register  reg,
                       typename T::CType* values,
                       size_t             count) const;
...
void Session::readOrWrite(NiFpgaEx_Register        reg,
                          ...
                          const size_t             count) const
{
   ...
   if ((IsSingle || count == 1) && T::elementBytes <= 4 && personalityFile->isMapped())
   {
      ...
      typedef typename std::conditional<std::is_same<T, Sgl>::value, float, uint32_t>::type MappedType;
...
template <typename T>
void Session::read(const NiFpgaEx_Register reg,
                   typename T::CType&      value) const
{
...
template <typename T>
void Session::write(const NiFpgaEx_Register reg,
                    typename T::CType       value) const
{
   readOrWrite<T, true, true>(reg,
                              const_cast<typename T::CType*>(&value),
                              1);
}
...
void Session::writeArray(const NiFpgaEx_RegisterArray   reg,
                         ...
                         const size_t                   count) const
{
   readOrWrite<T, false, true>(reg,
                               const_cast<typename T::CType*>(values),
                               count);
}
                                  ...
                                  size_t* const          elementsRemaining)
{
   ...
   // pass it on
   fifos[fifo]->acquire<T, IsWrite>(elements,
                                    elementsRequested,
                                    timeout,
                                    elementsAcquired,
                                    elementsRemaining);
