...
{
public:
   uint32_t getType() const { return MEMORY_TYPE_USER; }
...
public:
   ...
   explicit FifoBufferHost(size_t size) :
      buffer(valloc(size), free),
      size(size)
   {
      if (!buffer)
   ...
   void* getBuffer() const { return buffer.get(); }
...
{
public:
   FifoBufferGpu(void *buffer, size_t size) :
      buffer(buffer),
      size(size)
   {
   }

   uint32_t getType() const { return MEMORY_TYPE_NVIDIA; }
   size_t getSize() const { return size; }
...
Fifo::Fifo(const FifoInfo&    fifo,
           const std::string& device) :
   FifoInfo(fifo),
   device(device),
   acquireRelease(FifoSysfsFile(device,
				number,
				"nirio_supports_fifo_acquire_release",
				errnoMap).readBool()),
   started(false),
   hardwareElementBytes(FifoSysfsFile(device,
                                      number,
                                      "nirio_element_bytes",
                                      errnoMap).readU32()),
   paddingBytes(FifoSysfsFile(device,
                              number,
                              "nirio_padding_bytes",
                              errnoMap).readU32()),
   ...
   releaseFile(  device, number, "nirio_release_elements",   errnoMap)
{
   ...
   assert(acquireRelease != FifoSysfsFile(device,
                                          number,
                                          "nirio_supports_fifo_read_write",
                                          errnoMap).readBool());
}
                               ...
                               const bool   gpu) const
{
   ...
   actualSize = pageRound(requestedDepth * hardwareElementBytes + paddingBytes,
                          gpu);
...
void Fifo::configure(const size_t  requestedDepth,
                     size_t* const actualDepth)
{
   configureCpuOrGpu(requestedDepth, actualDepth, NULL);
}

void Fifo::configureGpu(const size_t depth,
                        void* const  buffer)
...
void Fifo::configureCpuOrGpu(const size_t  requestedDepth,
                             size_t* const actualDepth,
                             void* const   gpuBuffer)

{
   ...
   else
   {
      ...
      // otherwise, open the cdev file for the first time
         ...
         file.reset(new DeviceFile(DeviceFile::getFifoCdevPath(device,
                                                               number),
                                   hostToTarget ? DeviceFile::WriteOnly
                                                : DeviceFile::ReadOnly,
                                   errnoMap));
...
void Fifo::release(const size_t elements)
{
   ...
   if (!gpu)
   {
      ...
      {
         VALGRIND_MAKE_MEM_NOACCESS(buf + (nextInBytes - acquiredInBytes), releasingInBytes);
      }
      else
      {
                           ...
                           size_t* const  elementsRemaining)
{
   ...
   fifo_wait.wait_num_elem = elementsRequested,
   fifo_wait.timeout_ms = timeoutMs;
   try
   {
...
size_t Fifo::pollUntilAvailable(const size_t  elementsRequested,
                                size_t* const elementsAvailable,
                                const Timer&  timer)
{
   assert(file && started);
