...
void parseVersionString(rapidxml::xml_node<>& element,
                        unsigned int&         major,
                        unsigned int&         minor)
{
   major = 0;
...
rapidxml::xml_node<>& findFirstChild(rapidxml::xml_node<>& parent,
                                     const char* const     name = NULL)
{
   if (auto* const child = parent.first_node(name))
      return *child;
   else
      throw rapidxml::parse_error("XML parse error: child element not found",
                                  NULL);
}

rapidxml::xml_attribute<>& findFirstAttribute(rapidxml::xml_node<>& parent,
                                              const char* const     name)
{
   if (auto* const attribute = parent.first_attribute(name))
      return *attribute;
   else
      throw rapidxml::parse_error("XML parse error: attribute not found",
                                  NULL);
}
   ...
   bitstreamVersion(invalid)
{
   try
   {
      ...
      parseVersionString(xmlBitfile / "BitfileVersion",
                         major,
                         minor);
      ...
      autoRunWhenDownloaded =
         parseBoolean(xmlProject / "AutoRunWhenDownloaded");
      // find the base address
      auto& xmlNiFpga = xmlProject
                      / "CompilationResultsTree"
                      / "CompilationResults"
                      / "NiFpga";
      baseAddressOnDevice =
         parseUnsignedInteger(xmlNiFpga / "BaseAddressOnDevice");
      // get the bitstream version
      bitstreamVersion =
         parseUnsignedInteger(xmlBitfile / "BitstreamVersion");
           ...
           xmlChannel = xmlChannel->next_sibling("Channel"), ++i)
      {
         ...
         const std::string name =
            findFirstAttribute(*xmlChannel, "name").value();
         const auto number =
            parseUnsignedInteger(*xmlChannel / "Number");
         const auto controlSet =
            parseUnsignedInteger(*xmlChannel / "ControlSet");
         ...
         // determine type
         const auto type =
            parseType((*xmlChannel / "DataType" / "SubType").value());
      ...
      if (xmlRegisterBlockList)
              ...
              xmlRegisterBlock = xmlRegisterBlock->next_sibling("RegisterBlock"))
         {
            // find the offset
            const auto offset =
               parseUnsignedInteger(*xmlRegisterBlock / "Offset");
            // find the name
