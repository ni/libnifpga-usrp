...
int accessToOpenFlag(const DeviceFile::Access access)
{
   switch (access)
   {
      ...
      case DeviceFile::ReadOnly:  return O_RDONLY;
      case DeviceFile::WriteOnly: return O_WRONLY;
      case DeviceFile::ReadWrite: return O_RDWR;
   }
}
...
int accessToMmapProtection(const DeviceFile::Access access)
{
   ...
   {
      default:                    assert(false); // fall through
      case DeviceFile::ReadOnly:  return PROT_READ;
      case DeviceFile::WriteOnly: return PROT_WRITE;
      case DeviceFile::ReadWrite: return PROT_READ | PROT_WRITE;
   }
}
   ...
   errnoMap(errnoMap)
{
   ...
   do
   {
      ...
      descriptor = ::open(path.c_str(),
                          accessToOpenFlag(access) | O_CLOEXEC);
...
size_t DeviceFile::read(void* const  buffer,
...
size_t DeviceFile::write(const void* const buffer,
                         const size_t      size) const
{
...
void DeviceFile::ioctl(const unsigned long int request,
...
void DeviceFile::mapMemory(const size_t size)
{
   ...
   // try the mapping
   mapped = static_cast<volatile uint8_t*>(mmap(NULL,
                                           size,
                                           accessToMmapProtection(access),
                                           MAP_SHARED,
                                           descriptor,
                                           0));
...
std::string DeviceFile::getFifoCdevPath(const std::string&     device,
