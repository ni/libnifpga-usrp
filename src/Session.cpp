...
const class : public ErrnoMap
{
   public:
      virtual void throwErrno(const int error) const
      {
         switch (error)
         {
            case EALREADY: throw FpgaAlreadyRunningException();
...
Session::Session(const std::string& bitfilePath,
   ...
   baseAddressOnDevice(bitfile.getBaseAddressOnDevice())
{
   ...
   for (auto it  = bitfile.getFifos().cbegin(),
             end = bitfile.getFifos().cend(); it != end; ++it)
   {
...
bool Session::run() const
{
   ...
   try
   {
      PersonalitySysfsFile(device,
                           "nirio_run_vi",
                           alreadyErrnoMap).write(true);
   }
...
bool Session::download(bool force)
{
   ...
   try
   {
      boardFile.write(personalityBlob->getBlob(),
   ...
   personalityFile.reset(new DeviceFile(DeviceFile::getCdevPath(device,
								"personality"), DeviceFile::ReadWrite));
                           ...
                           NiFpgaEx_Resource&          resource) const
{
   ...
   if (isRegister(type))
   {
      ...
      for (auto it  = bitfile.getRegisters().cbegin(),
                end = bitfile.getRegisters().cend(); it != end; ++it)
      {
         const auto& reg = *it;
         // if we found a match
...
void Session::waitOnIrqs(uint32_t irqs,
                         uint32_t timeout,
                         uint32_t* const irqsAsserted,
                         bool* timedOut)
{
   PersonalitySysfsFile(device, "nirio_irq_mask").write(irqs);
   DeviceFile irqStatus(std::string("/sys/class/nirio/") + device + "!personality/nirio_irq_status",
                        DeviceFile::ReadOnly);
...
void Session::configureFifo(const NiFpgaEx_DmaFifo fifo,
...
void Session::configureFifoGpu(const NiFpgaEx_DmaFifo fifo,
                               const size_t           depth,
                               void* const            buffer)
...
void Session::releaseFifoElements(const NiFpgaEx_DmaFifo fifo,
